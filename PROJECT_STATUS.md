...
### 2026-09-20 日常リンクを通常のセルリンクへ変更

- `daily_view.link()`をHYPERLINK数式から表示文字列＋`userEnteredFormat.textFormat.link`へ変更。確認・履歴・ホーム・設定を既存refreshで再生成し、旧編集画面への初期リンクも同じ送信maskへ統一する。固定Spreadsheet ID / gid / range、文言、列、確認判断を保持する。
- 差分更新と読戻しは型付きセル値とリンクURIを照合し、同じラベルのURL変更・リンク解除・応答喪失後の再実行に対応。書式更新はfontSizeだけを指定し、リンクを消さない。通常セルの書式/入力規則と本人入力は対象外。
- 関連33件、Windows full pytest 2,520件成功（120.67秒、依存ライブラリの既存警告2件）。本番は既存projection専用scopeを1回使用する。正本read-only、取込/確認送信/会計stageは起動しない。実本番readbackとiOS実機確認はこのコミット時点では未実施。

### 2026-09-20 正式台帳ホームを編集・管理へ整理

- 正式台帳「家計簿AI」の旧カテゴリグラフ1個と旧集計表示を外し、自動分類ルール・個別修正・過去分プレビュー/確認・各種確認待ち・原本/設定へ進むメニューに変更した。グラフは日常ファイルの2個を維持。個別修正等の既存フォームへ直接リンクし、別のwriterを作らない。
- 一度限りの表示変更builderを追加。変更対象は正本ホームA1:B34のうちB3/B4以外と既知chartのみ。分類操作A1:L1274の全入力/規則/メモ、ホームの月選択と依存helperを読戻して保持を確認。取引データ、原本、共有、保護、Payroll/Medical、定期実行は変更していない。過去分リンクはsection見出しから動的に行を解決する。
- 関連47テスト成功。所有/binding確認・変更範囲・入力保持・chart削除の再実行・未知chartでの停止を検証。実ファイルの退避とread-backは`.private/operations-*`へ保存。旧ホームUIの再インストールはcompact移行後すでに拒否されるため、定期処理で旧グラフを再生成しない。

### 2026-09-20 日常ホームへ月別推移・カテゴリ内訳を追加

- ホームの既存情報の下に325px幅のnative chartを縦に2個配置。直近13か月の折れ線と、B3へ即時連動する対象月の大カテゴリ横棒（上位5＋残り）を追加した。データ源は既存summary/catalog、既存非表示セルのみ。6タブ12,630セルを維持し、未集計は空白、記録済みの差引0円・負の返金は維持する。
- 2025-09〜2026-09の全13候補を実日常ファイルで切り替え、金額/件数とカテゴリ6枠すべてが保存済み集計に一致。元の対象月と日付型を復元。グラフ表示・位置とnative specの読戻しを確認し、同じspecの再適用が不要なことも照合した。本人入力・候補・確認内容・正式台帳・共有・定期フラグは変更対象外。
- 固定chart IDの差分更新とread-backを実装し、定期refreshで増殖しない。カテゴリ合計保存、未集計/差引0、既存chart保持、API default/RGB丸め、書込応答喪失後の再開とread-back不一致検出を合成テストに追加。実数値/snapshotはGit対象外`.private/daily-charts-*`へ保存。

### 2026-09-20 日常ホームの対象月を即時連動へ修正

- 原因はrendererがホームB4/B5へ選択月時点の固定値を書いていたこと。B3がSheetsの日付serialとして保存されるケースも実データで確認。既存summaryから13か月の金額/件数/取込状況を非表示`_候補!A987:D1000`へ保存し、B2/B4/B5を日付・文字列両対応の厳密XLOOKUPへ変更した。正本全明細の参照/再計算、Apps Script、新サービスは不要。定期rendererも同じ数式を維持する。
- カテゴリ/固定ID候補は1ページ最大501件のまま、候補書込領域をA2:D986へ限定し、月別参照表と重ならないようにした。6タブ12,630セルを維持。取込状況未確認を完了扱いせず、未集計/未確認の空月は0へ変換しない。確認が必要は従来どおり全期間の未解決件数。
- 実日常ファイルへ同じコードの5 native requestを適用。2026-08/09の実プルダウン切替・再読込と、2025-09〜2026-09の全13候補を逐次変更してGoogle SheetsのeffectiveValueを保存済みsummaryへ照合し全一致。検証後B3の元の日付型/選択月を復元。全シート比較で変更はhelper56セル＋ホーム4セルだけ、本人入力43セル・その他値/書式/入力規則/確認内容を保持。実金額と検証snapshotはGit対象外`.private/home-month-*`に保存。
- 関連56テスト成功。月替わり・欠損/未確認0件・完了0件・購入/返金の差引0円・13か月参照・カテゴリページとの非干渉・式を値へ戻さないreplayを検証。定期/本番フラグ、共有、正式台帳、Payroll/Medical設定は変更していない。
