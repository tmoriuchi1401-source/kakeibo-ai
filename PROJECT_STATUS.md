...
- **残課題:** mainへのコード統合。最新main `cbb7c8e`に別作業の銀行定期処理/本番書込み権限変更があり、取込みが自動承認レビューで拒否されたため保留。業務値や月選択の再開前からの更新は上書きしない。カテゴリ手修正は次の既存支出一覧refreshで反映し、source再取込時のauthorityは従来どおり。旧カテゴリ補助シートは保持。
- **次の1作業:** レビュー可能なUIコミットについて、上記mainを基点とする統合承認を得て、統合状態で検証してからmainへ反映する。UI確認目的の本番業務処理は起動しない。
