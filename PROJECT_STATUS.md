...
### Google Sheets 日常UI（本番適用・検証完了）

- **Level:** 利用者承認後に本番UIを適用。native数値・構造照合、fixture検証、390px幅での実画面目視確認を完了。
- **完成範囲:** ホーム1枚、先頭の日常3画面、内部台帳6枚の非表示、入力色/書式/寸法の整理。対象月プルダウンをUI生成コードへ正式同期し、「要対応」に対象月のカテゴリ未分類件数/金額と全期間の通常review・Amazon reviewを区別して表示。dry-run既定の再実行処理、UIだけの復元、既存refreshへの導入済み時限定の書式補正。
...
- **再実行/復元:** B4の選択を保持し、B3の表示月を再生成。選択月の未読/不正/同時変更は適用前に停止する。同期後の再dry-runでホーム/グラフ/結合の新規作成0、B4の再書込みなし。今回のUI復元情報37 requestsをGit除外のprivate領域へ保存。旧B3日付入力の移行と補助I列/メモ/固定行の復元をfixture検証。全体pytest **1089 passed**（UI **22 passed**）、compileall・diff-check成功。
...
- **残課題:** このUI範囲の必須検証は完了。元台帳と支出一覧の更新差は既存ビューの反映待ちとして残り、ホームは支出一覧の反映時点を示す。旧カテゴリ補助シートは廃止調査候補として保持。
- **次の1作業:** 通常の既存refreshに合わせて利用。UI確認目的の本番業務処理は起動しない。
