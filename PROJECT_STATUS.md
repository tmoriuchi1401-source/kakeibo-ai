...
### 2026-09-16 一般・Medicalの確認受付接続（本番反映前）
...
- 合成で未入力拒否、入力保持、別プロセス再開、確定反映/重複0、既存支出へのリンク、保存失敗、
  ...
  本番UIの受付開始と実記帳は、この実装検証とは別に本番読戻しで記録する。
- 実装SHA `db223e76e191ee70141aafd00e9588961f5a3ffd` のLinux CI `35040728936` 成功。
  一般1370＋機微合成70＝1440件、Node 6件、compileall/diff-check成功。本番Secrets/実帳票なし。
  追加の局所確認で、反映直前に入力が変わり会計callがまだ0なら再確認へ戻し、既存メモも保持する。
- 本番の読取だけで固定対象版と既存台帳を照合した表示案は一般4行＋Medical1行。
  変更不要6件、本人入力欄は全て空欄。案は非公開領域の `confirmation-20260916/ui-draft.json`。
  Windows shadow検証を繰り返した結果ではなく、新しい確認UI生成処理による表示案の検証。
- 本番反映前の一時停止が自動承認レビューで実行前に拒否された。最新Goalの停止/反映/再開の承認文と
  稼働run 0件の証拠を提示しても「Medical確認機能を本番反映する目的は明示承認範囲外」と再拒否。
  他経路で実行していない。main/承認SHAは `c4039a80c7259cbfdef79506972c781d14a44b86` のまま、
  新親/定期ON・旧日常OFF・共通保守active。専用Sheetsタブは未作成、確認受付は未開始、実記帳未確認。
  再開にはこのMedical確認受付導入の一時停止・検証済みmain統合・binding/承認SHA設定・本番検証・再開への明示承認が必要。
