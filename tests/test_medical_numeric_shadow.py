...
def test_no_production_import_or_resolution_change():
    ...
    for path in (Path(__file__).resolve().parents[1]/'app').glob('*.py'):
        if path.name in {'medical_numeric_shadow.py', 'medical_numeric_multipass.py', 'medical_region_shadow.py'}:
            continue
        for node in ast.walk(ast.parse(path.read_text(encoding='utf-8'))):
            ...
            assert not any(module in name for name in names for module in ('medical_numeric_shadow', 'medical_numeric_multipass', 'medical_region_shadow'))
