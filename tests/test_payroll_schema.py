...
def test_apply_creates_five_sheets_headers_and_only_initial_master_rows():
    ...
    assert len(plan.initial_standard_item_ids) == len(INITIAL_STANDARD_ITEMS) == 25
    assert len(plan.initial_alias_ids) == len(INITIAL_ALIASES) == 21
    ...
    assert result["inserted_standard_item_count"] == 25
    assert result["inserted_alias_count"] == 21
...
def test_apply_requires_explicit_confirmation_and_preview_never_writes():
    ...
    assert output["initial_standard_item_count"] == 25
    assert output["initial_alias_count"] == 21
    assert service.state["operations"] == []
