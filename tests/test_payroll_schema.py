...
def test_apply_creates_five_sheets_headers_and_only_initial_master_rows():
    ...
    assert len(plan.initial_standard_item_ids) == len(INITIAL_STANDARD_ITEMS) == 24
    assert len(plan.initial_alias_ids) == len(INITIAL_ALIASES) == 20
    ...
    assert result["inserted_standard_item_count"] == 24
    assert result["inserted_alias_count"] == 20
...
def test_apply_requires_explicit_confirmation_and_preview_never_writes():
    ...
    assert output["initial_standard_item_count"] == 24
    assert output["initial_alias_count"] == 20
    assert service.state["operations"] == []
