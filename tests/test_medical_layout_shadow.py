    ...
    for path in (Path(__file__).resolve().parents[1] / "app").glob("*.py"):
        ...
        assert not any(isinstance(n, ast.ImportFrom) and
                       any(s in (n.module or "") for s in
                           ("medical_layout_shadow", "medical_layout_evaluation", "medical_layout_local", "medical_numeric_shadow", "medical_numeric_multipass", "medical_region_shadow", "medical_receipt_unit_shadow"))
