...
- 金銭語の部分一致・近似・分割文字を候補とし、全候補の囲み・内容・残存画素を個別検査する。
  元の切出し未解決理由と件数は保持し、会計上の役割は別の `medical-accounting-roles-v1` で評価する。
  同じラベル画素からの重複検出、見出し/印欄、負担率、保険者負担、点数、明確な前回入金を区別する。
  非支払欄の数字の正読や送信可能な画像の生成は、会計上の役割を確認する必須条件にしない。
  「今回請求」と「今回入金」は対象時点・ラベル列・共通の表境界が確認できる場合だけ対応づける。
  保険分/内税の内訳も親の請求・入金欄との配置関係を要求する。請求/未収/預り/返金/累計は名前だけで除外しない。
  別の支払欄、部分入金、別領収書、未対応の金銭欄や通貨値は会計競合として保留する。
...
- 自動記帳は、対応する非AI日付/施設、マスタ内カテゴリ、一意な実支払額、署名済み保存応答、
  ...
  支払方法は推測せず空欄とする。前後31日以内の同額支出/取込、または日付不明の同額行は重複不明として保留する。
  安全性を検証した画像の解析は、他候補の切出し失敗や日付不足と分離する。
  会計上の未解決競合が0で、独立した今回支払欄が1つの場合だけ、他の記帳条件へ進む。
...
通常policy `auto-v1:free` へ接続済みで、今回の保留は他のMedical/一般/他sourceを停止しない。
