...
G列「対象資料」とH列「比較資料」には、確認理由に応じた短いリンクを表示する。
レシートはDriveのfile IDで原本画像・PDFを開き、その他の取込は元の取込行、
確定した統合先があれば既存の支出明細または関連取込行へ直接移動する。
明細合計とレシート合計の不一致が取込備考に記録された行では、
H列「明細合計を見る」からその合計が記載された取込行のLセルへ直接移動する。
重複候補が複数ある場合は「確認材料」の該当候補一覧を開く。
対象が特定できない場合は推測したリンクを出さない。詳細は
[確認材料の設計](docs/review-evidence.md)を参照。
