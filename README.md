...
- 金銭語の部分一致・近似・分割文字を候補とし、全候補の囲み・内容・残存画素を個別検査する。
  未解決理由と候補件数は専用storeへ保存。未解決候補や複数の支払欄を無視して1つへ昇格しない。
  既知の前回/累計入金・未収・預り金・釣銭は、同じ厳格な領域/文字検査に通った場合だけ非支払欄と区別する。
  汎用の請求額・合計だけでは実支払額を確定しない。
...
- 自動記帳は、対応する非AI日付/施設、マスタ内カテゴリ、一意な実支払額、署名済み保存応答、
  ...
  安全性を検証した画像の解析は、他候補の未解決や日付不足と分離する。未解決候補・複数の支払欄が残れば記帳しない。
