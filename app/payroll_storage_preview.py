    ...
    for raw_candidate in candidates:
        ...
        plans.append(PayrollSavePlan(
            ...
            recognized_item_count=sum(
                item.standard_item_id is not None for item in candidate.items
            ),
            unknown_item_count=sum(
                item.standard_item_id is None for item in candidate.items
            ),
            needs_review_count=sum(item.needs_review for item in candidate.items),
            duplicate_status=duplicate_status,
            duplicate_reason=(duplicate.reason if duplicate.reason != "none" else None),
            would_create_header=can_create,
            would_create_items=len(candidate.items) if can_create else 0,
