"""Production persistence adapters with the write capability still sealed.

SQLite is used for local Windows and single-runner GitHub Actions durability.
The real Sheets adapter is deliberately unreachable because capability issuance
is disabled in this phase and no CLI imports this module.
"""
...
from .aupay_card_apply_plan import CanonicalApplyCandidate, CanonicalApplyPlan
...
class ProductionWriteCapability:
    ...
    def __new__(cls, *args, **kwargs):
        raise RuntimeError("production_capability_issuance_disabled")


def issue_production_write_capability(*_args, **_kwargs) -> ProductionWriteCapability:
    """Hard-disabled until a separately approved canary phase."""
    raise RuntimeError("production_capability_issuance_disabled")
...
def evaluate_capability_seal(checks: CapabilitySealChecks) -> tuple[str, ...]:
    """Return privacy-safe blockers. This phase always remains sealed."""
    blockers = [
    ...
    blockers.append("production_capability_disabled")
    return tuple(blockers)


class SealedSheetsCandidateTransport:
    """Exact single-candidate Sheets append adapter, sealed in this phase."""
    ...
    def write_once(self, candidate: CanonicalApplyCandidate) -> WriteRequestResult:
        ...
        if (
            not PRODUCTION_CAPABILITY_ENABLED
            or type(capability) is not ProductionWriteCapability
            or not capability.explicit_apply
        ):
            raise RuntimeError("production_capability_issuance_disabled")
        validate_target_binding(
            self._binding, self._inspector.inspect(self._binding.expected_worksheet),
        )
        if candidate.identity not in capability.allowed_identities:
