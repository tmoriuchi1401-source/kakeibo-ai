...
Amazonカード照合は、商品単価ではなく注文ID単位の合計金額で判定するよう修正した。既存30件を再判定し、一意一致16件・複数候補1件・注文履歴なし13件となった。候補なしは `amazon_unmatched` として中優先度へ分離した。現在の要確認は高1件（Amazon複数候補）、中155件、計156件。
