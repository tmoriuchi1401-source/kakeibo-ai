...
要確認シートでは、大カテゴリに応じて小カテゴリ候補を絞る入力規則を追加済み。非表示の `_要確認カテゴリ候補` シートでは、要確認の各行に対応する `FILTER` 式を横方向へ展開し、小カテゴリの入力規則はその固定範囲を参照する。2026-08-17 に実シートへ反映し、入力規則の読み取り検証に成功した。現在の要確認は高17件（Amazon曖昧候補）、中161件、計178件。
