...
| 支出明細 | 0 | 右側、表示。F:Gを入力色 | 支出元台帳。receipt / Amazon / auto-expense / review等の既存経路は維持。Fはカテゴリマスタ由来の大カテゴリ、Gは同じ行のFに連動した小カテゴリを選ぶ。カテゴリ対応から既存F:Gへ直接移動し、人が明示入力する |
...
| _支出明細カテゴリ候補 | 1909140004 | 非表示、UI所有 | カテゴリ!A:Bだけから数式で大カテゴリと行別小カテゴリ候補を作る。固定コピーではなく、F:Gの入力規則だけが参照する |

業務シートの名前、既存sheetId、ヘッダ、列の物理順序、業務値、判断値、共有・保護は変更しない。例外として支出明細F:Gだけは、本書のカテゴリ連動入力規則を明示的に復旧する。
UI所有のホームとカテゴリ対応は、表示文言・数式・補助列を再生成する。入力規則の追加対象はホームB4と支出明細F:Gだけである。
...
支出明細のF:Gは合計315px、淡い黄色、ヘッダ固定。F2:F5001はカテゴリマスタから生成した重複なしの大カテゴリ、適用時点の既存G行は同じ行のFに一致するカテゴリマスタの小カテゴリだけを候補にする厳格なnativeドロップダウンである。通常の`SheetsDB.append("支出明細", ...)`は新規行だけに同じ行別G規則を追加するため、5,000行上限内の取込追加にも適用される。カテゴリマスタの変更は補助数式を通じて候補へ反映され、カテゴリ値の固定コピーを持たない。大カテゴリの変更で既存小カテゴリを自動消去しないが、候補外の既存値は厳格な規則により不適合として表示され、再選択が必要になる。手修正は次の既存支出一覧refreshで表示へ反映される。元sourceの再取込等による更新仕様は変更しない。0件でもホームのリンクを残し、カテゴリ対応には「この月の未分類はありません。」と表示する。review 0件時も既存画面へ移動できる。
...
`SheetsDB.configure_review_validation()`と`format_date_column()`に小さな書式補正だけを追加した。支出明細F:Gの復旧は明示的なUI applyでだけ行い、通常の業務writerや支出一覧refreshが入力規則を消去・上書きしない。
...
applyは再読取で契約/配置が変わっていないことを確認し、UI復元情報をGit除外の`.private`へ排他的に保存してから、限定batchUpdateを1回送る。送信の自動retryは行わない。カテゴリ復旧のapplyは、UI所有の非表示補助シート、支出明細の空行確保、F:Gのdata validationだけを追加・更新する。支出明細の値・数式・メモ・既存列は変更しない。
...
業務シートの復元対象は元のタブ順/表示、日常3シートと支出明細F:Gの今回変更した書式・幅・行高・固定行、F:Gの適用前data validationだけ。
**業務データをバックアップで全体上書きしない。** 復元はF:Gの以前の入力規則だけを戻し、値・数式・メモ・共有・保護には書き込まない。
