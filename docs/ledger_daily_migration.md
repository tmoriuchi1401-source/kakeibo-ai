...
正式台帳「家計簿AI」のホームは、日常のグラフ表示から編集・管理用メニューへ変更した。日常の記録・グラフは「家計簿AI 日常」に集約し、正式台帳からは自動分類ルール、個別修正、過去分プレビュー/反映確認、一般/領収書/Amazon金銭の確認、登録済みルールや原本へ進める。個別修正・カテゴリ名管理・金銭確認は既存の日常フォームへ直接リンクし、送信経路を増やさない。

`ledger_operations_home.build_plan`はcutover済みの正本/日常bindingと所有markerを確認した上で、一度だけ明示適用する表示変更を生成する。ホームA1:B34の表示と既知chart 1909140002だけが対象。B3/B4の選択・数式・入力規則と、既存カテゴリ操作の月候補が依存するD:Iは保持する。分類操作のsection 2/3へのリンクは見出しをMATCHして求め、行数増加によるリンク切れを避ける。通常の本番処理には組み込まず、取引や入力済みの承認を実行しない。旧UI installerはcompact移行後すでに無効なので、通常更新でグラフが復活する経路はない。

適用前のnative chart spec/位置・ホームのセル/書式/行高・正本metadataとカテゴリ操作入力はGit対象外`.private/operations-*`に保存。復旧はホームの変更範囲/行高/結合と当該chartだけを戻し、その時点のB3/B4・D:I・カテゴリ操作入力・取引データを保持する。

### ホームのグラフ

ホームA14に選択月の大カテゴリ内訳（上位5件＋その他）、A31に直近13か月の支出推移のnative chartを配置（2026-09-21に月間を上へ変更）。幅325pxで縦に並べ、既存の入力・案内を覆わない。A13には選択月と取込状況、A30には記録済み分・未集計月の案内を表示。29行目を32pxにし、高さ340pxの上段グラフと下段見出しの間隔を確保する。内訳はB3のプルダウン変更だけで切り替わる。金額は記録済み分で、取込完了を意味しない。未集計月は空白、記録のある差引0円や負の返金は数値を維持する。

データ源は既存summaryとcatalogのみ。推移の既存非表示列D1:E14に月別値、D20:F98に13か月×6枠のカテゴリ集計、D102:E108にXLOOKUPによる選択月の表示値を置く。正本全明細の参照・外部スクリプト・追加セルは不要。43入力と候補表は対象外。固定chart IDで新規作成・内容変更・位置変更を判定し、定期refreshでも重複追加しない。APIが補完するdefaultやRGBの丸めを考慮してread-backする。

復旧はchart ID 260920101/260920102、ホーム13/30行と上記helper領域だけを今回のprivate snapshotへ戻す。本人の現在のB3、正式台帳、確認フォームを復元対象にしない。

### ホーム対象月の即時連動

ホームB3を変更すると、B2の取込状況とB4/B5の記録済み支出/買い物件数はGoogle Sheetsの数式で自動更新する。既存月別summaryの直近13行だけを非表示`_候補!A988:D1000`へ書き、日付serialなら`TEXT(B3,"yyyy-mm")`で検索キーを正規化する。厳密一致のXLOOKUPで金額/件数/状態を取得し、未取得を0で補完しない。定期処理はこの参照表のデータと数式を保守する。B6は全期間の未解決件数なので選択月に連動させない。

この修正は12,630セルの既存grid内で完結し、本人入力も正式台帳も書き換えない。13候補すべてのGoogle再計算結果と既存summaryが一致し、2026-08/09は実プルダウン操作と再読み込みでも確認した。snapshotはprivateに保持。復旧が必要なら本人の現在のB3を保持し、home専用出力と末尾14行だけを修正前snapshotから戻す。正本や他のフォームを復元対象にしない。
