...
ホームA14に直近13か月の支出推移、A31に選択月の大カテゴリ内訳（上位5件＋その他）のnative chartを配置。幅325pxで縦に並べ、既存の入力・案内を覆わない。A30には選択月と取込状況を表示し、内訳はB3のプルダウン変更だけで切り替わる。金額は記録済み分で、取込完了を意味しない。未集計月は空白、記録のある差引0円や負の返金は数値を維持する。
